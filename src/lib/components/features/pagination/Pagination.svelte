...
<div class="flex flex-row items-center justify-between gap-2 text-xs">
	...
	<div class="join">
		<button
			...
			disabled={page <= 1}
			onclick={() => updateQueryParam([{ key: 'page', value: page - 1 }, { key: 'size', value: size }])}
		...
		<select
			class="select select-sm select-primary w-fit min-w-24 join-item"
			value={page}
			onchange={(e) =>
				updateQueryParam([
	...
	<div class="flex w-36 flex-row items-center justify-end gap-2">
		<select
			class="select select-sm select-primary w-fit min-w-16"
			value={size}
			onchange={(e) =>
				updateQueryParam([
					{ key: 'page', value: 1 },
