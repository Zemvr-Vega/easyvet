...
<div class="flex h-full w-full flex-col">
	...
	<div class="ev-fade-up grid grid-cols-3 gap-3 p-5 pb-0">
		...
		<div class="ev-panel flex items-center gap-3 p-4">
			...
			<div>
				<p class="text-xs tracking-wider text-base-content/50 uppercase">Scheduled</p>
							...
							<tr class="hover:bg-base-200">
								...
								<td>
									<p class="font-semibold">
										{date.formatDate({ date: s.scheduled_at, format: 'MMM dd, yyyy' })}
									</p>
									<p class="text-base-content/40">
										{date.formatDate({ date: s.scheduled_at, format: 'hh:mm aa' })}
									</p>
								</td>
								<td>Dr. {s.surgeon}</td>
								...
								<td>
									<form method="POST" action="?/update_status" use:enhance>
										...
										>
											{#each statuses as st}
												<option value={st} selected={s.status === st} class="capitalize"
													>{st}</option
												>
											{/each}
										</select>
		...
		<div class="modal-box max-h-[90vh] max-w-2xl overflow-y-auto">
			...
			>
				...
				<fieldset class="fieldset">
					...
					<select name="type" class="select w-full select-sm" required>
				...
				<fieldset class="fieldset">
					...
					<select name="anesthesia_type" class="select w-full select-sm">
				...
				<fieldset class="col-span-2 fieldset">
					...
					<textarea
						...
						rows="2"
						placeholder="Fasting instructions, pre-medication..."
					></textarea>
				</fieldset>
