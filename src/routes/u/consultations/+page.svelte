...
<div class="flex h-full w-full flex-col">
	<div class="ev-fade-up grid grid-cols-3 gap-3 p-5 pb-0">
		<div class="ev-panel flex items-center gap-3 p-4">
			...
			<div>
				<p class="text-xs tracking-wider text-base-content/50 uppercase">Today</p>
		...
		<div class="ev-panel flex items-center gap-3 p-4">
			...
			<div>
				<p class="text-xs tracking-wider text-base-content/50 uppercase">Pending</p>
		...
		<div class="ev-panel flex items-center gap-3 p-4">
			...
			<div>
				<p class="text-xs tracking-wider text-base-content/50 uppercase">Completed</p>
				<p class="text-xl font-bold">{data.stats.total_completed}</p>
			</div>
		</div>
	</div>

	<div class="ev-page min-h-0 grow">
		<header class="flex items-center justify-between gap-4">
			<div class="flex items-center gap-2">
				...
				{#if status_filter}
					<a href="/u/consultations" class="btn text-xs opacity-60 btn-ghost btn-sm">Clear</a>
				{/if}
			</div>
			<button
					...
					<tbody class="z-10 text-xs">
						{#each data.consultations as c}
							<tr class="hover:bg-base-200">
								...
								<td>
									<span class="badge badge-xs {type_badge[c.type] ?? 'badge-ghost'} capitalize"
										>{c.type}</span
									>
								</td>
								<td>
									<p class="font-semibold">
										{date.formatDate({ date: c.scheduled_at, format: 'MMM dd, yyyy' })}
									</p>
									<p class="text-base-content/40">
										{date.formatDate({ date: c.scheduled_at, format: 'hh:mm aa' })}
									</p>
								</td>
						...
						{:else}
							<tr>
								<td colspan="9" class="py-12 text-center text-sm text-base-content/40"
									>No consultations found.</td
								>
							</tr>
						{/each}
					</tbody>
	...
	<div class="modal-open modal">
		<div class="modal-box max-w-lg">
			<div class="mb-4 flex items-center justify-between">
				...
				<button onclick={() => (show_modal = false)} class="btn btn-circle btn-ghost btn-xs">
					<X class="size-4" />
				</button>
			</div>
			...
			<form
				...
				use:enhance={() => {
					return async ({ result, update }) => {
						if (result.type === 'success') toast.success('Consultation created.');
						if (result.type === 'failure') toast.error('Failed to create consultation.');
			...
			>
				...
				<fieldset class="fieldset">
					...
					<select name="type" class="select w-full select-sm">
				...
				<fieldset class="col-span-2 fieldset">
					...
					<textarea
						...
						rows="2"
						placeholder="Medical history, owner observations..."
					></textarea>
				</fieldset>
