...
<div class="flex h-full w-full flex-col">
	<!-- Stats -->
	<div class="ev-fade-up grid grid-cols-2 gap-3 p-5 pb-0">
		<div class="ev-panel flex items-center gap-3 p-4">
			...
			<div>
				<p class="text-xs tracking-wider text-base-content/50 uppercase">Pending Tests</p>
					...
					<tbody class="z-10 text-xs">
						{#each data.tests as t}
							<tr class="hover:bg-base-200">
								...
								<td class="text-xs uppercase">{t.test_type}</td>
								...
								<td>
									{#if t.status !== 'completed' && t.status !== 'cancelled'}
										<button
											...
											onclick={() => {
												result_target = { id: t._id, name: t.test_name, status: t.status };
						...
						{:else}
							<tr>
								<td colspan="10" class="py-12 text-center text-sm text-base-content/40"
									>No lab tests found.</td
								>
							</tr>
						{/each}
					</tbody>
		...
		<div class="modal-box max-w-lg">
			...
			>
				...
				<fieldset class="fieldset">
					...
					<select name="test_type" class="select w-full select-sm" required>
				...
				<fieldset class="col-span-2 fieldset">
					...
					<textarea name="notes" class="textarea w-full textarea-sm" rows="2"></textarea>
				</fieldset>
...
{#if show_result_modal && result_target}
	<div class="modal-open modal">
		<div class="modal-box max-w-md">
			<h3 class="mb-4 text-lg font-bold">Update — {result_target.name}</h3>
			<form
				method="POST"
			...
			>
				...
				<fieldset class="fieldset">
					...
					<select name="status" class="select w-full select-sm">
						{#each statuses as s}
				...
				<fieldset class="fieldset">
					...
					<input
						...
						name="performed_by"
						class="input input-sm w-full"
						placeholder="Lab technician / Vet"
					/>
				</fieldset>
				<fieldset class="fieldset">
