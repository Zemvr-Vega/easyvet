<script lang="ts">
	...
	const role_badge: Record<string,string> = {
		admin: 'badge-error', veterinarian: 'badge-primary', receptionist: 'badge-success',
		technician: 'badge-info', viewer: 'badge-ghost'
	};
	const status_badge: Record<string,string> = {
		active: 'badge-success', inactive: 'badge-ghost', suspended: 'badge-error'
	};
	...
	<div class="flex items-center justify-between">
		<form onsubmit={(e) => { e.preventDefault(); doSearch(); }} class="flex items-center gap-2">
			<div class="ev-search-wrap w-52">
				<Search />
				<input type="text" class="ev-search" placeholder="Search users..." bind:value={search_input} />
			</div>
						...
						<tr>
							<td>
								<div class="flex items-center gap-2.5">
									<div class="flex h-7 w-7 items-center justify-center rounded-full bg-primary/15 text-xs font-bold text-primary flex-shrink-0">
										{user.firstname[0]}{user.lastname[0]}
									</div>
									<p class="font-semibold text-base-content text-sm">{user.firstname} {user.lastname}</p>
								</div>
							</td>
							<td class="text-base-content/60 text-xs">{user.email}</td>
							<td class="text-base-content/60 text-xs">{user.phone || '—'}</td>
							<td>
								<span class="badge badge-xs {role_badge[user.role] ?? 'badge-ghost'} capitalize">{user.role}</span>
							</td>
							<td>
								<span class="badge badge-xs {status_badge[user.status]} capitalize">{user.status}</span>
							</td>
							...
							<td>
								<div class="flex items-center gap-1">
									...
									<form method="POST" action="?/update_role" use:enhance>
										...
										<select name="role" class="select select-xs"
									...
									<form method="POST" action="?/update_status" use:enhance>
										...
										<select name="status" class="select select-xs"
											onchange={(e) => e.currentTarget.form?.requestSubmit()}>
											{#each statuses as s}
												<option value={s} selected={user.status === s} class="capitalize">{s}</option>
											{/each}
										</select>
									</form>
									<!-- Archive -->
									<form method="POST" action="?/archive" use:enhance>
										...
										<button type="submit" class="btn btn-xs btn-ghost text-error/60 hover:text-error"
											onclick={(e) => { if (!confirm(`Archive ${user.firstname}?`)) e.preventDefault(); }}>
											<X class="size-3" />
										</button>
									</form>
								</div>
		...
		<div class="modal-box max-w-md">
			...
			<form method="POST" action="?/create" use:enhance={() => {
				...
				<fieldset class="fieldset">
					...
					<input id="firstname" name="firstname" type="text" class="input input-sm w-full" placeholder="Juan" required />
				</fieldset>
				<fieldset class="fieldset">
					<label class="label" for="lastname">Last Name <span class="text-error">*</span></label>
					<input id="lastname" name="lastname" type="text" class="input input-sm w-full" placeholder="Dela Cruz" required />
				</fieldset>
				<fieldset class="fieldset col-span-2">
					<label class="label" for="email">Email Address <span class="text-error">*</span></label>
					<input id="email" name="email" type="email" class="input input-sm w-full" placeholder="user@easyvet.com" required />
				</fieldset>
				<fieldset class="fieldset">
					<label class="label" for="phone">Phone</label>
					<input id="phone" name="phone" type="text" class="input input-sm w-full" placeholder="09XX-XXX-XXXX" />
				</fieldset>
				<fieldset class="fieldset">
					<label class="label" for="role">Role</label>
					<select id="role" name="role" class="select select-sm w-full">
						{#each roles as r}
							<option value={r} class="capitalize">{r}</option>
						{/each}
					</select>
				</fieldset>
				<p class="col-span-2 text-xs text-base-content/40 bg-base-200 rounded-lg px-3 py-2">
